...
The deposited labels themselves reproduce every count the paper states exactly: 73 clusters; the seven OPC/oligodendrocyte clusters sum to 10,087; clusters 55–73 (non-neuronal) to 27,096; CGC clusters 25 + 28 to 15,360; cerebellar interneuron clusters 24, 26, 27, 29 to 1,890. These are identity checks on the authors' output, so they are not counted as coverage. The cluster-number boundaries above are confirmed by the authors' own cluster names (Supplementary Table S4's column headers, e.g. `55 Oligo MFOL2` … `61 OPC`, `68 Astro Slc7a10` … `71 Bergmann Glia`; fetched from PMC7643870 — see below), not inferred from the counts alone.
...
* **Attempted fix: real marker genes instead of textbook panels (not adopted).** The paper's closed-access status previously blocked getting its Methods; the full text (via PMC7643870, `Data/rosenberg2018/supplement/pmc7643870.xml`) turned out to be fetchable, and it names the exact supplement for cell-class assignment: "Each of these 73 clusters was assigned to a cell class on the basis of expression of established marker genes (Tables S3–S5)." Table S4 (top-50 DE genes per author cluster, `Benchmarks/rosenberg2018_splitseq/Supplement/`, not committed — see License below) lets each of the 6 non-neuronal class panels above be replaced with the authors' own reported markers (genes recurring across a majority of a class's member clusters), instead of a textbook guess; the neuron panel can't be improved this way since pan-neuronal genes are never "differentially expressed" against an atlas that's already 74% neuronal, so they never surface in a per-cluster DE list. `atlas_reclass_v2.py` reuses the same de novo Leiden clustering (unaffected by marker choice) and reclassifies with these panels: astrocyte-of-non-neuronal moves from failing (40.6%) to a marginal pass (43.0%, threshold 42.5%), but non-neuronal fraction barely moves (34.5%, still ~2x the paper's 17.4%), oligo/CGC stay off, and cerebellar interneuron — previously passing — regresses to 0.4% (paper 1.21%). Net: still 4 failing checks, just not the same 4, and one of the flips is a hair's-width pass on one seed. Not adopted; kept as `Docs/SPLiTseq/*_atlas_v2_table_s4_panels/` for the record. The result reinforces the section's real conclusion: the gap isn't panel wording, it's that a de novo Leiden clustering + any marker-score classifier doesn't cleanly recover this atlas's fine cell-class proportions — only the authors' own (unpublished) clustering does.
...
Scripts: `fig1_metrics.py` (Fig. 1, via the port), `prep_atlas.py` (GEO atlas → AnnData with deposited labels), `atlas_recluster.py` (Figs. 2, 4, 5), `fig3_allen_regions.py` (Fig. 3). `make_figures.py` and the old figure belong to the earlier subsample benchmark and are no longer run. `atlas_reclass_v2.py` is the not-adopted marker-panel experiment above; it reads `Benchmarks/rosenberg2018_splitseq/Supplement/NIHMS1581077-supplement-TableS4.xlsx` (fetch from PMC7643870 first, not committed) and the `atlas_leiden_calls.tsv.gz` of an existing `atlas_recluster.py` run.
...
* **Data**: GEO GSE110823 (public), Allen Developing Mouse Brain Atlas API (public); fetched at run time, never redistributed. The paper's full text and Supplementary Materials (PMC7643870, closed-access publisher copyright) were fetched for reference only — `pmc7643870.xml` and the Supplement tables/PDF are gitignored, not redistributed.
