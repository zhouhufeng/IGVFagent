...
|---|---|---|---|---|---|
| ⭐ | **Matreyek 2018** PTEN VAMP-seq *Nat Genet* | `mavedb` | **4/4 concordance checks pass** (8,000 variants, all CDS-mapped) | **full** — job `J2026092322583678f071`, MCP tools only (no shell) | suite-verified smoke-test |
| 1 | **Waters 2024** BAP1 SGE *Nat Genet* | `mavedb` (new SGE path) | **LOF +1.1 %, GOF +7.7 %** vs paper | CLI (`run.sh`) | [`waters2024_bap1/`](waters2024_bap1/README.md) |
| 2 | **Buckley 2024** VHL SGE *Nat Genet* | `mavedb` (new SGE path) | **2,268 / 2,268** variants recovered, 4-bucket scheme | CLI (`run.sh`) | [`buckley2024_vhl/`](buckley2024_vhl/README.md) |
| 3 | **Zou 2024** ChIP-Atlas 3.0 *Nucleic Acids Res* | `chipatlas` | **815 TFs catalogued** for hg38/Blood, **GATA1 rank #7** (121 experiments) | CLI (`run.sh`) | [`zou2024_chipatlas_gata1/`](zou2024_chipatlas_gata1/README.md) |
| 4 | **Agarwal 2025** lentiMPRA K562/HepG2/WTC11 *Nature* | `mpra` (`pull`) | **3/3 discovery artefacts** written; portal manifest + summary JSON | CLI (`run.sh`) | [`agarwal2025_lentimpra/`](agarwal2025_lentimpra/README.md) |
| 5 | **Yao 2024** ENCODE4 noncoding CRISPRi *Nat Methods* | `encode` (new FCE path) | **368 CRISPR-screen FCEs enumerated**; Engreitz 65 %, K562 dominant, GATA1 +24/+58 kb spot-check ✓ | **partial** — Catalog cross-checks at GATA1, MYC, FADS1/2 | [`yao2024_encode4_crispri/`](yao2024_encode4_crispri/README.md) |
| 6 | **Mitra 2024** SCARlink multi-ome regression *Nat Genet* | `multiome peak2gene` (**full local repro**) | **Genome-wide peak→gene scan on the paper's own BMMC subset** (GSE194122): 7,642 cells, 10/10 donors and 5/5 cell types match the paper's list; 923,107 candidate pairs, 16,650 significant; **4/4 checks**. The IGVF Portal survey (505 multiome AnalysisSets) is kept in `portal_survey/` | CLI (`run.sh`) | [`mitra2024_multi_regression/`](mitra2024_multi_regression/README.md) |
| 7 | **Weinstock 2024** CD4 T-cell CRISPR network *Cell Genomics* | `perturb-catalog` + `geo` + LLCB port | **1,197 CRISPR-screen datasets catalogued** (99.7 % CRISPRn, matching the KO design). `llcb_py/`, a Python port of the authors' LLCB (Julia) method fit on their own GSE271788 counts (311 samples, 84 KO genes), recovers a signed 84-gene network 3.5–4× denser than the paper's (877 vs 211 edges at \|β\|>0.025) | CLI (`run.sh`) | [`weinstock2024_cd4_crispr/`](weinstock2024_cd4_crispr/README.md) |
| 8 | **Zheng 2024** in-vivo Perturb-seq cortex *Cell* | `geo` + `sc-analyze` (Seurat object) | **Fig 4F reproduced from the paper's own Seurat object** (GSE249416): Foxg1-gRNA1 lowers L6-IT 8.2-fold (paper 9.9-fold) and raises upper-layer neurons 1.3-fold (paper 2.0-fold), same direction; **4/4 checks** | CLI (`run.sh`) | [`zheng2024_invivo_perturbseq/`](zheng2024_invivo_perturbseq/README.md) |
| 9 | **Martyn 2025** Rewriting regulatory DNA (Variant-EFFECTS) *Cell* | `flowfish` + `portal` | **5/5 analyses on real IGVF Portal data** (13/13 checks): 88 significant PPIF promoter + enhancer variants vs the paper's 89; PPIF enhancer 60,784 bp upstream (paper ~60.5 kb); all 3 splice-site edits recovered; lentiMPRA arm scored. The citation here was fabricated earlier and is now corrected | **partial** — Portal datasets (PPIF, IL2RA, lentiMPRA arm) via `portal_get` | [`martyn2025_variant_flowfish/`](martyn2025_variant_flowfish/README.md) |
| 10 | **Southard 2025** TF Perturb-seq fibroblasts *Nat Genet* (was mislabeled "Joung 2025" — see [correction](southard2024_comprehensive_transcription/perturb_catalog/README.md)) | `perturb-catalog` + real data verification | **Guide-library counts confirmed against the authors' own deposited data**: 10,979 guides and 1,836 TF targets, both exact matches, measured directly from Zenodo 10.5281/zenodo.15200179 rather than trusted from prose. The old `joung2025_tf_perturbseq` catalog step now lives in `perturb_catalog/` | CLI (`run.sh`) | [`southard2024_comprehensive_transcription/`](southard2024_comprehensive_transcription/README.md) |
| 11 | **Deng 2024** cortex lentiMPRA *Science* | `mpra` + new `synapse` | **Full Synapse deposit recovered**: 166-node walk of NeuREs (`syn21392931`); 96 paired DNA+RNA files in MPRA_CapstoneII enumerated; all 12 paper-asserted annotations recovered; download step waits only on user-side PsychENCODE DUA + PAT | CLI (`run.sh`) | [`deng2024_cortex_mpra/`](deng2024_cortex_mpra/README.md) |
| 12 | **Gschwind 2023 / Sheth 2024** ENCODE-rE2G & scE2G *bioRxiv* | `portal` + `synapse` + new `e2g_benchmark_eval` | **ENCODE-rE2G base AUPRC 0.634 reproduces the published 0.634 exactly** vs the K562 CRISPR ground truth; ranking rE2G-extended (0.758) > rE2G-base (0.634) > scE2G (0.53–0.59) > ARC-E2G ≈ ABC (0.49); scE2G beats the ABC baseline as claimed. Same two preprints as the scE2G crowdsourced-features row, different analysis | CLI (`run.sh`) | [`e2g_crispr_benchmark/`](e2g_crispr_benchmark/README.md) |
| 13 | **Travaglini 2020** Human Lung Cell Atlas *Nature* | `sc-analyze` (**full local repro**) | **49 Leiden clusters vs 46 author cell types**; AMI 0.81, homogeneity 0.89; 9/10 canonical lung markers peak in expected cell type; **8/8 checks** | CLI (`run.sh`) | [`travaglini2020_lung/`](travaglini2020_lung/README.md) |
| 14 | **Trevino 2021** developing cortex multiome *Cell* | `multiome peak2gene` (**full local repro**) | **cis peak→gene links for all 26 lineage genes; 93 % positive** (enhancer→gene activation), TSS-proximal; reproduces the paper's genome-wide linkage signal; **4/4 checks** | CLI (`run.sh`) | [`trevino2021_cortex_multiome/`](trevino2021_cortex_multiome/README.md) |
| 15 | **deMULTIplex2 / Stoeckius 2018** cell hashing *Genome Biol* | `multiseq` (**full local repro**) | **`bench score`: 7/13 analyses** (24/24 checks pass). On the bundled PBMC matrix: all 8 donor HTO groups from 15,113 cells, 83 % singlets, pool balance 0.79. The other 6 analyses need ground-truth labels that were never deposited | CLI (`run.sh`) | [`demultiplex2_stoeckius/`](demultiplex2_stoeckius/README.md) |
| 16 | **Rosenberg 2018** SPLiT-seq developing CNS *Science* | `splitseq` (**full local repro**) | **`bench score`: 7/12 analyses** (16/22 checks). 156,049-nucleus atlas parsed exactly; all 8 CNS lineages recovered | CLI (`run.sh`) | [`rosenberg2018_splitseq/`](rosenberg2018_splitseq/README.md) |
| 17 | **Ma 2020** SHARE-seq mouse skin *Cell* | `share` + shared `_scload` (**full local repro**) | **`bench score`: 7/10 analyses**. 34,774-cell skin set (exact); all 23 author cell types; Leiden AMI 0.63 on shallow SHARE-seq RNA | CLI (`run.sh`) | [`ma2020_shareseq/`](ma2020_shareseq/README.md) |
| 18 | **Wang 2025** developing neocortex multiome *Nature* | `sc-analyze` (**full local repro**) | **232,328-nucleus atlas (exact); 29 author cell types**; Leiden AMI 0.65, homogeneity 0.70; **7/7 checks** | CLI (`run.sh`) | [`wang2025_neocortex_multiome/`](wang2025_neocortex_multiome/README.md) |
| 19 | **Zou/Shi 2026** scEPS GWAS × single-cell *medRxiv* | new `sceps` port (**full local repro**) | **Microglia significantly AD-associated: aggregated d=4.76e-5, Z=4.99, P=6e-7** on SEA-AD + Bellenguez-AD MAGMA; AD-GWAS genes ≫ matched controls; port matches upstream corr 1.0 | CLI (`run.sh`) | [`zou2026_sceps/`](zou2026_sceps/README.md) |
| 20 | **Wang 2026** Spatial-ATAC-Hi-C *Nat Methods* | new `spatial-hic` port (**offline, planted ground truth**) | **All planted structure recovered**: CN gain 96% of true fold with 4.39× clone separation, clone-specific loop p<1e-300 in the right clone with no null-loop false positive, A/B blocks 93.3%, TSS enrichment exact; contact stats inside the paper's bands; **18/18 checks** (+3 unconfirmed paper values). **Part 2** on the paper's own GEO data ([`wang2026_spatial_atac_hic_geo/`](wang2026_spatial_atac_hic_geo/README.md)): 1/12 checks so far | CLI (`run.sh`) | [`wang2026_spatial_atac_hic/`](wang2026_spatial_atac_hic/README.md) |
| 21 | **Tabula Sapiens Consortium 2026** reference human cell atlas *Cell* | new `tabula` + `humantfs` ports (**full local repro**) | **Dataset description exact**: 1,136,218 cells (1,093,048 droplet / 43,170 FACS), 24 donors, 28 tissues, 182 fine / 38 broad types, 701 populations, ages 22-74, 11M/13F. **TF analysis within 1%**: 882/741 specific vs non-specific (paper 890/745) over the paper's own 175 cell types, and exactly SHOX+ZBED1 with zero expression atlas-wide. 70 GO terms vs 69. Also documents that the paper's Fig 3 background differs from its Methods, and that the 103 TB raw bucket is DTA-gated; **26/26 checks** | CLI (`run.sh`) | [`quake2026_tabula_sapiens/`](quake2026_tabula_sapiens/README.md) |
| 22 | **Ryu 2024** crispr-bean base-editing variant effects *Nat Genet* | new `bean-benchmark` + `bean` (real `bean` handoff) | **Data-level claims reproduce from the authors' own Zenodo deposit**: replicate Spearman ρ **0.878 vs 0.88** (tiling) and **0.831 vs 0.84** (GWAS), mean per-gRNA edit fraction **33.9–34.5% vs 34.0%**, tiling library **7,500 gRNAs and 150 non-targeting controls exact**. **5 agree / 6 differ / 7 await the model fit** — five of the six differences are one cause (the deposits are post-QC, the paper's counts pre-filter); the sixth, median maximal editing 0.496 vs 0.604, is open. Establishes that the IGVF-deposited Sherwood screens are **not** this paper's screens. AUPRC comparison blocked pending a replicate-count question for the authors | CLI (`run.sh`) | [`ryu2024_crispr_bean/`](ryu2024_crispr_bean/README.md) |
| 23 | **IGVF scE2G working group 2026** crowdsourced K562 E2G features *Synapse deposit* + Sheth 2024 scE2G | new `sce2g` workbench (`benchmark --all-features`, `inventory`, `merge`) + `synapse` + `portal`, **checked against `CRISPR_comparison` itself** | **121 single features from 16 Synapse tables scored against the K562 CRISPR ground truth, and the scorer validated against the field's own pipeline run on the same inputs**: per-pair scores identical for all 10,356 pairs, AUPRC equal to 4 decimals on its definition (rE2G base 0.6326 = published 0.634; scE2G 0.5190; Pinloop 0.3311). **No single feature beats distance** (0.418): Pinloop 0.357, Signac 0.227, SCENT beta 0.175, EPCOT EP300-at-element 0.159 carry signal; conservation, constraint, Alu, motif density and ChromHMM state are at random (0.046). The gold-standard run also caught two scorer bugs (inverse-predictor fill, tie handling) that were fixed; **27/27 checks**. Same two preprints as the ENCODE-rE2G & scE2G row, different analysis | CLI (`run.sh`) | [`sce2g_crowdsourced_features_k562/`](sce2g_crowdsourced_features_k562/README.md) |
| 24 | **Liu 2025** kidney multiome genetic scorecard *Science* | new `liu2025-gwas-loci` + `liu2025-open4gene` ports + `figshare` | **`bench score`: 11/16 analyses** (43/46 checks). Both ports match the authors' code (awk/R GWAS filter + loci merge; `pscl::hurdle` zero β/p identical). ASE genes, bASA, snASA, Open4Gene links and scorecard counts are exact. 3 checks fail: bASA–ASE correlation 0.31 vs 0.51, snASA in peaks 87 % vs 93 %, median peaks/gene 4 vs 8. COJO step pending; primary data controlled access | CLI (`run.sh`) | [`liu2025_kidney_multiome/`](liu2025_kidney_multiome/README.md) |
| 25 | **Rosen 2025** MPRAsnakeflow *Genome Res* | `mpraflow` | **The authors' own MPRAsnakeflow output reproduced exactly**: IGVF lentiMPRA AnalysisSet IGVFDS1933XFAF, 1,900,181 barcodes in → all 35,110 rows × 6 columns identical to the authors' IGVFFI3491STGH; **8/8 checks** | CLI (`run.sh`) | [`rosen2025_mprasnakeflow/`](rosen2025_mprasnakeflow/README.md) |
