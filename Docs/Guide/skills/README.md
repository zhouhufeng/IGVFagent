...
| `paper-code` | Reproduce a paper by running the authors' own code: repository from the Code Availability statement, pinned commit, R/Python environment, unmodified execution, comparison with the authors' rendered output, replay check, per-section report; claims-first checks of key results against the published tables with a seed noise floor | [Reproducing a paper](../paper-reproduction.md) | [`paper_code_skill.py`](../../../Scripts/paper_code_skill.py) |
